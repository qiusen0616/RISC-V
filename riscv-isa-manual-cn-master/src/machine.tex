...
\begin{commentary}
  机器级中断寄存器处理一些根中断源，为简单起见，这些根中断源被分配了固定的服务优先级，
  而每一个外部中断控制器可以在更大的一组中断上实现更复杂的优先级方案，这些中断随后被多路复用到机器级中断源中。
\end{commentary}
...
\begin{commentary}
不可屏蔽中断不需要通过 {\tt mip} 寄存器来实现可见，因为在执行NMI陷阱处理程序时会隐式知道它的存在。
\end{commentary}
...
位 {\tt mip}.MEIP 和 {\tt mie}.MEIE 是机器级外部中断的中断挂起和中断使能位。 
...
如果实现了监管器模式，{\tt mip}.STIP和{\tt mie}.STIE位是监管级时钟中断的中断挂起和中断使能位。
STIP在{\tt mip}中是可写的，并且可以由M模式软件写入以将时钟中断传递到S模式。

如果实现了监管器模式，{\tt mip}.SSIP和{\tt mie}.SSIE位是监管级软件中断的中断挂起和中断使能位。 
SSIP在{\tt mip}中是可写的，也可以由特定于平台的中断控制器设置为1。

以M模式为目标的多个同时发生的中断按以下优先级降序处理：MEI、MSI、MTI、SEI、SSI、STI。
...
当MXLEN=32时，读取{\tt mcycle}、{\tt minstret}和{\tt mhpmcounter{\em n}} CSR会返回相应计数器的31--0位，
...
当{\tt mcounteren}寄存器中的 CY、TM、IR或HPM{\em n}位清零时，
...
\subsection{机器模式临时存储寄存器 ({\tt mscratch})}
...
{\tt mepc} 是一个 \warl\ 寄存器，它必须能够保存所有有效的虚拟地址。 
它不需要能够保存所有可能的无效地址。 
